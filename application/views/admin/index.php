                ...
                <table class="table table-striped bg-white">
                    ...
                    <?php foreach ($post as $post) : ?>
                        <tr>
                            ...
                            <td width="200px"><button onclick="window.location.href = '<?= base_url('admin/preview_post/') . $post['id_post'] . '/' . $post['id_jenis_post'] ?>'" class="btn-info mr-1">Preview</button><button onclick="confirm('apakah anda yakin menyetujui post ini?'); window.location.href = '<?= base_url('admin/approve_post/') . $post['id_post'] ?>'" class="btn-success mr-1">setujui</button><button onclick="confirm('apakah anda yakin akan menghapus post ini?'); window.location.href = '<?= base_url('admin/reject_post/') . $post['id_post'] ?>'" class="btn-danger">tolak</button></td>
                        </tr>
                    <?php endforeach ?>
                </table>
            </div>
